---
title: Gas Bad NFT Marketplace Audit Report
author: SnavOhBurmaa
date: June 6, 2026
---
...
- [SnavOhBurmaa](https://www.linkedin.com/in/khant-wai-yan-ohburmaa/)
...
- [Gas Bad NFT Marketplace Audit Report](#gas-bad-nft-marketplace-audit-report)
...
- [Findings](#findings)
  - [Medium](#medium)
    - [\[M-1\] NFTs sent directly to the marketplace via `safeTransferFrom` are permanently locked](#m-1-nfts-sent-directly-to-the-marketplace-via-safetransferfrom-are-permanently-locked)
    - [\[M-2\] `buyItem` credits `msg.value` instead of `price`, so buyer overpayment is silently captured by the seller](#m-2-buyitem-credits-msgvalue-instead-of-price-so-buyer-overpayment-is-silently-captured-by-the-seller)
  - [Low](#low)
    - [\[L-1\] `withdrawProceeds` emits no event](#l-1-withdrawproceeds-emits-no-event)
    - [\[L-2\] No validation that `nftAddress` is a contract / a real ERC-721](#l-2-no-validation-that-nftaddress-is-a-contract--a-real-erc-721)
  - [Gas / QA](#gas--qa)
    - [\[G-1\] `<= 0` comparisons on `uint256` are misleading and slightly wasteful](#g-1--0-comparisons-on-uint256-are-misleading-and-slightly-wasteful)
    - [\[G-2\] `buyItem` re-reads the listing struct into multiple locals](#g-2-buyitem-re-reads-the-listing-struct-into-multiple-locals)
  - [Informational](#informational)
    - [\[I-1\] Acknowledged known issues (front-running / MEV)](#i-1-acknowledged-known-issues-front-running--mev)
    - [\[I-2\] PUSH0 opcode limits deployment to PUSH0-enabled chains](#i-2-push0-opcode-limits-deployment-to-push0-enabled-chains)
    - [\[I-3\] Static-analysis false positive: "state change without event" on assembly emitters](#i-3-static-analysis-false-positive-state-change-without-event-on-assembly-emitters)
- [Formal Verification (Certora)](#formal-verification-certora)
- [Static Analysis (Slither, Aderyn)](#static-analysis-slither-aderyn)
- [Appendix A — PoC Test Harness](#appendix-a--poc-test-harness)
</details>
</br>
...
1. **Manual review** of both contracts, line-by-line, with special attention to the
   inline-assembly event emitters in `GasBadNftMarketplace.sol`.
2. **Differential analysis** between the reference and optimized contracts (the whole
   point of the project is that they must behave identically).
3. **Formal verification** with Certora (equivalence rule + an "every storage write
   emits an event" invariant).
4. **Static analysis** with Slither and Aderyn, plus an independent recomputation of
   every hard coded `keccak256` event topic with `cast keccak`.

# Protocol Summary

The Gas Bad NFT Marketplace is an escrow-based ERC-721 marketplace. A seller lists an
NFT (which is transferred into the marketplace), a buyer pays ETH to receive it, and the
seller later pulls their proceeds. The project ships **two** implementations:

- `NftMarketplace.sol` — a plain solidity *reference* implementation.
- `GasBadNftMarketplace.sol` — a gas optimized version that emits all four events using
  raw `log4` opcodes in inline assembly instead of Solidity `emit`. It is the contract
  intended for mainnet deployment.

The two are meant to be *behaviourally identical*; Certora's equivalence rule proves this
at the storage level.

| Function | Purpose |
| --- | --- |
| `listItem` | Escrow an NFT and create a listing (`price > 0`). |
| `cancelListing` | Seller withdraws their unsold NFT. |
| `updateListing` | Seller changes the price. |
| `buyItem` | Buyer pays ETH, receives the NFT, seller is credited. |
| `withdrawProceeds` | Seller pulls accumulated ETH. |

## Roles

- **Seller:** Lists, updates, and cancels NFTs, and withdraws proceeds.
- **Buyer:** Pays ETH to receive a listed NFT.
- There is **no owner/admin role** and **no upgradeability** — the contract is immutable
  once deployed
